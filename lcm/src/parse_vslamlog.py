...
class TestLcmLog(object):
    def __init__(self):
        ...
        self.lc = lcm.LCM("file:///home/levin/temp/MirrorData_localiation_log.log?mode={}&speed=15".format(self.mode))
        return
    def run(self):
        if self.mode == "w":
            self.send()
...
#             self.lc.publish("EXAMPLE2", msg2.encode())
        ...
        return
    def recv(self):
        subscription = self.lc.subscribe("cam_front", self.my_handler)
        try:
            while True:
                self.lc.handle()
