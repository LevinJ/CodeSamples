...
class PoseInfo(object):
    ...
    def output_xy_yaw(self):
        ...
        yaw = self.ypr[0]
        return [x,y,yaw]
    def output_xyz_yawpitchroll(self):
        res = []
        res.extend(self.t)
        res.extend(self.ypr)
        return res
    def output_xyz_yawpitchroll_string(self):
        t = self.t
        ypr = self.ypr*180/math.pi
        res = "[{:.2f}, {:.2f}, {:.2f}], [{:.1f}, {:.1f}, {:.1f}]".format(t[0], t[1], t[2], ypr[0], ypr[1], ypr[2])
        return res
    def output_xyz_q(self):
        res = []
        res.extend(self.t)
        res.extend(self.q)
        return res
    ...
    def get_short_desc(self, tree_dim = True):
        ...
        if tree_dim:
            t = self.t
            ypr = self.ypr * 180/math.pi
            res = "t=[{},{},{}],ypr=[{},{},{}]".format(t[0], t[1],t[2],ypr[0],ypr[1],ypr[2])
        return res
